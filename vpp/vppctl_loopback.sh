...
if [[ "$#" -eq 0 ]]; then
	...
	sudo $BINS/vppctl -s $PREFIX set int l2 xconnect $NAMELC0P1 $NAMELC0P3
	sudo $BINS/vppctl -s $PREFIX set int l2 xconnect $NAMELC0P3 $NAMELC0P1
else
    ...
    sudo $BINS/vppctl -s $PREFIX test l2patch rx $NAMELC0P1 tx $NAMELC0P3
    sudo $BINS/vppctl -s $PREFIX test l2patch rx $NAMELC0P3 tx $NAMELC0P1
fi
