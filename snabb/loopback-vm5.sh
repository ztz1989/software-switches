...
cd path/to/qemu
sudo taskset -c 32-35 ./bin/x86_64-softmmu/qemu-system-x86_64 -name $VM_NAME -cpu host -enable-kvm \
   -m $GUEST_MEM -drive file=$CDROM --nographic \
  -numa node,memdev=mem -mem-prealloc -smp sockets=1,cores=4 \
  -object memory-backend-file,id=mem,size=$GUEST_MEM,mem-path=/dev/hugepages,share=on \
  -chardev socket,id=char8,path=$VHOST_SOCK_DIR/vhost-user-9,server \
  ...
  -chardev socket,id=char9,path=$VHOST_SOCK_DIR/vhost-user-10,server \
